...
| [ADR-004](#adr-004-wake-word--picovoice-porcupine-wakewordprovider-adapter-arkasinda--2026-08-24) → **`docs/decisions/ADR-004-wake-word-provider.md`** | Wake word motoru = **sherpa-onnx `KeywordSpotter`** (Rust + `cpal`, offline; %2.3 CPU / 38 MB idle, Apache-2.0). Porcupine elendi | **accepted (kapsami daraltilmis)** — motor+yerlesim kesin; **model+ifade secimi ACIK**: `gigaspeech-3.3M` "Hey Asuna"yi tasimiyor (sozlukte yok, %0 tespit). Ilk adim: gercek mikrofon testi (`spike/asu-008b-kws` branch'i). Phase 2 bloklu, Phase 1 etkilenmez |
